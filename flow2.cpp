...
def main():
        ...
        # for j in range(0,len(discreteTimeWindows[i])): 
            for k in range(0,len(discreteTimeWindows)):
                ...
                if (i!=k and discreteTimeWindows[i][len(discreteTimeWindows[i])-1]+timeMatrix[i][k]<discreteTimeWindows[k][0]):
                        ...
                        n=0
                        print("time window:", discreteTimeWindows[i])
                        for m in range(int(intime-discreteTimeWindows[i][0]),len(discreteTimeWindows[i])):
                            if(n<len(discreteTimeWindows[k])):
                                print(m)
                                u = nodeCount[i][m]
    ...
    for i in range(0,len(nodeCount)):
        # for j in range(0,len(nodeCount[i])):
            x[edgeNumber] =  solver.NumVar(0, 1, 'x[%i]' % edgeNumber)
            edges['x[%i]' % edgeNumber] = (nodeCount[i][len(nodeCount[i])-1],-1)
            edgeFlows[edgeNumber] = (nodeCount[i][len(nodeCount[i])-1],-1)
            #add edge number in edge list 
            edgesList[nodeCount[i][len(nodeCount[i])-1]].append(edgeNumber)
            edgeNumber+=1
    edgesList.append([])
